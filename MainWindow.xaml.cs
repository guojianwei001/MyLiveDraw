...
namespace AntFu7.LiveDraw
{
    public partial class MainWindow : Window
    {
        ...
        //for angle draw
        private Point? _vertex, _startPoint;      // 顶点、第一条边的结束点
        ...
        public MainWindow()
        {
            if (mutex.WaitOne(TimeSpan.Zero, true))
            {
                ...
                MainInkCanvas.SelectionChanging += MainInkCanvas_SelectionChanging;
        ...
        private void MainInkCanvas_SelectionResizing(object sender, InkCanvasSelectionEditingEventArgs e)
        {
            //throw new NotImplementedException();
            //Display("MainInkCanvas_SelectionResizing");

            Stroke tmpStroke = MainInkCanvas.GetSelectedStrokes().First();
            int pointCount = tmpStroke.StylusPoints.Count();
            Display("&&pointCount is " + pointCount);
        }

        private void MainInkCanvas_SelectionResized(object sender, EventArgs e)
        {
            //throw new NotImplementedException();
            //没有选中任何textBlock，返回
            if (0 == MainInkCanvas.GetSelectedElements().Count)
                return;

            int countStrokes = MainInkCanvas.GetSelectedStrokes().Count;
            ...
            if (countStrokes == 1)
            {
                ...
                //判断是否是rec和ellipse，只有这两种情况才更新size
                if (pointCount != 5 || pointCount != 2 * Math.PI)
                    return;

                double area = tmpStroke.GetGeometry().GetArea();
                //Display("&&Area is " + area + "mm2");

                int countUIElements = MainInkCanvas.GetSelectedElements().Count;
                if (countUIElements >= 1)
                {
                    UIElement uIElement = MainInkCanvas.GetSelectedElements().First();
                    Display("uIElement.GetType().Name" + uIElement.GetType().Name + " Area is " + area + "mm2");
                    if (uIElement.GetType().Name == "TextBlock")
                    {
                        ((TextBlock)uIElement).Text = area + "mm2";
                    }
                }
            }
        ...
        private void MainInkCanvas_SelectionChanging(object sender, InkCanvasSelectionChangingEventArgs e)
        {
            //throw new NotImplementedException();
            //Display("MainInkCanvas_SelectionChanging");
        }

        private void MainInkCanvas_SelectionChanged(object sender, EventArgs e)
        {
            // throw new NotImplementedException();
            Display(e.GetType().ToString() + "##SelectionChanged##" + e.ToString());

            if (MainInkCanvas.GetSelectedStrokes().Count > 1)
            {
                //选中stroke大于一个的时候，不能多个同时resize
                MainInkCanvas.ResizeEnabled = false;
            }
            else
            {
                MainInkCanvas.ResizeEnabled = true;
            }
        }
        ...
        private void MainInkCanvas_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            var drawingAttributes = this.MainInkCanvas.DefaultDrawingAttributes.Clone();
            ...
            if (_mode == DrawMode.Ray)
            {
                if (!_vertex.HasValue) // 画角度时第一次按下鼠标
                {
                    _vertex = _drawerIntPos; // 存第一个点:顶点
                }
                else if ((_drawerIntPos - _vertex.Value).Length > minWidth && !_startPoint.HasValue) // 画角度时第二次按下鼠标 画第一条边
                {
                    _startPoint = _drawerIntPos; // 存第二个点:第一条边的结束点

                    var pointCollection = new StylusPointCollection(new Point[] { _vertex.Value, _startPoint.Value });
                    ...
                    {
                        this.MainInkCanvas.Strokes.Remove(_drawerLastStroke);
                    }

                    this.MainInkCanvas.Strokes.Add(stroke);
                    _drawerLastStroke = stroke;
                }
                else // 第三次按下鼠标画第二条边
                {
                    var pointCollection = new StylusPointCollection(new Point[] { _vertex.Value, _startPoint.Value, _vertex.Value, _drawerIntPos});
        ...
        private void MainInkCanvas_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else if (_mode == DrawMode.Ray)
            {
                ...
                drawingAttributes.Height = 2;
                drawingAttributes.FitToCurve = false;//must be false,other wise rectangle can not be drawed correct
                OnAngleMouseMove(endP, drawingAttributes);
                return;
            }
            ...
            else if (_mode == DrawMode.Arrow)
            {
                ...
                //Display($"the={theta:f1},s={_drawerIntPos.X:f1},{_drawerIntPos.Y:f1} e={endP.X:f1},{endP.Y:f1}, p={pointList[2].X:f1},{pointList[2].Y:f1}");
        ...
        private void OnAngleMouseMove(Point point, DrawingAttributes drawingAttributes)
        {
            if (_vertex.HasValue == false)
            {
                return;
            }

            if (_startPoint.HasValue == false) // 画第一条线
            {
                if ((point - _vertex.Value).Length < minWidth)
                {
                    return;
                }

                var pointCollection = new StylusPointCollection(new Point[] { _vertex.Value, point });
                var stroke = new Stroke(pointCollection)
                {
                    DrawingAttributes = drawingAttributes,
                };
            ...
            else // 画第二条线
            {
                ...
                var pointCollection = new StylusPointCollection(new Point[] { _startPoint.Value, _vertex.Value, point });
                var stroke = new Stroke(pointCollection)
                {
                    DrawingAttributes = drawingAttributes,
                };
